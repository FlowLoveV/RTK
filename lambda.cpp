//
// Created by 0-0 mashuo on 2023/3/26.
//
...
static int LD(int n, const double *Q, double *L, double *D)   //´Ë´¦·Ö½âËã·¨Í¬ FMFAC5£¨²Î¼û"Ä£ºý¶È¾­µä°æ.pdf"£©
{
    int i,j,k,info=0;
    double a,*A=mat(n,n); //mat():¸ø¾ØÕó·ÖÅäÄÚ´æ

    memcpy(A,Q,sizeof(double)*n*n); //½«QÕó¸´ÖÆµ½AÖÐ
    for (i=n-1;i>=0;i--) {          //FMFAC5ÖÐÊÇ´Ói=n¿ªÊ¼µÄ£¬ÎªºÎÓÐÇø±ð£¿ÓÉÓÚ¾ØÕóÔªËØÊÇ´Ó0µ½n*n-1£¬¶øFMFAC5ÖÐÊÇ1µ½n*n£¬¹Ê´Ë´¦ÐèÒª¶à¼õ1
        if ((D[i]=A[i+i*n])<=0.0) {info=-1; break;}  //D(i,i)=Q(i,i),×¢Òâ´Ë´¦DÊÇÒ»¸öÁÐÏòÁ¿¶ø·Ç¾ØÕó
        a=sqrt(D[i]);                                //a=sqrt(Q(i,i))
        for (j=0;j<=i;j++) L[i+j*n]=A[i+j*n]/a;      //L(i,1:i)=Q(i,1:i)/sqrt(Q(i,i))
        for (j=0;j<=i-1;j++) for (k=0;k<=j;k++) A[j+k*n]-=L[i+k*n]*L[i+j*n];  //Q(j,1:j)=Q(j,1:j)-L(i,1:j)L(i,j) Í¬ÑùµÄÓÉÓÚ¾ØÕóÔ­Òò£¬j´Ó0¿ªÊ¼
...
static void gauss(int n, double *L, double *Z, int i, int j)  //ÀàËÆZTRANËã·¨£¬µ«iºÍjµ÷»»ÁË,j±íÊ¾ÁÐ
{
    int k,mu;

    if ((mu=(int)ROUND(L[i+j*n]))!=0) {   //ËÄÉáÎåÈë
        for (k=i;k<n;k++) L[k+n*j]-=(double)mu*L[k+i*n];   //¸ßË¹ÏûÔª
        for (k=0;k<n;k++) Z[k+n*j]-=(double)mu*Z[k+i*n];   //Z¾ØÕó×öÏàÍ¬µÄ±ä»¯
    }
}

/* permutations½»»» --------------------------------------------------------------*/
static void perm(int n, double *L, double *D, int j, double del, double *Z)    //Ìõ¼þ·½²îÅÅÐò
...
static void reduction(int n, double *L, double *D, double *Z)  //ÀàËÆSRC1Ëã·¨£¬°Ñ¸ßË¹ÏûÔªºÍÌõ¼þ·½²îÅÅÐò·ÅÔÚÒ»Æð½øÐÐ£¨Ä£ºý¶È¾­µä°æ.pdf£©
{                                                              //ÎÒÈÏÎª¸ÃËã·¨ÓëSRC1Ö»ÊÇ¿´ÆðÀ´²»Í¬£¬µ«ÔËÐÐ¹ý³ÌÊÇÏàÍ¬µÄ
    int i,j,k;
    double del;

    j=n-2; k=n-2;    //´Ë´¦kÖ¸i1,ÓÉÓÚ¾ØÕóÔ­Òò±äÎªn-2   jÖ¸i,Í¬ÑùµÄÓÉÓÚ¾ØÕóÔ­Òò±äÎªn-2
    while (j>=0) {   //while i>1
        if (j<=k) for (i=j+1;i<n;i++) gauss(n,L,Z,i,j);   //ZTRANËã·¨
        del=D[j]+L[j+1+j*n]*L[j+1+j*n]*D[j+1];            //¼ÆËãdelta
        if (del+1E-6<D[j+1]) { /* compared considering numerical error */
            perm(n,L,D,j,del,Z);
            k=j; j=n-2;   //¼´i1=i£¬i¸³³õÖµÒÔ±ã½øÐÐÏÂÒ»´ÎÑ­»·¡££¨Ïà±ÈÓÚSRC1Ëã·¨ÉÙÁËsw²ÎÊý²¢ÇÒÉÙÁËswÄÇÒ»²ãÑ­»·£©
        }
        else j--;
    }
}
...
{                                                            //n:float parameters(nb=16)   m: fixed solutions(=2)   zs:¼´z=Z'*a
...
*          double *Q     I  covariance matrix of float parameters (n x n)  ¼´Qb£¨²»ÊÇ»ùÏßµÄ£¬¶øÊÇrtklibÀï¶Ô¹Ì¶¨½âµÄÃüÃû£©
*          double *F     O  fixed solutions (n x m)  ¼´b
...
extern int lambda(int n, int m, const double *a, const double *Q, double *F, double *s)
{
    ...
    if (n<=0||m<=0) return -1;
    L=zeros(n,n); D=mat(n,1); Z= lambda_eye(n); z=mat(n,1),E=mat(n,m);
